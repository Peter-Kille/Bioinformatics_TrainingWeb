...
this is shorthand for chmod (change modify) a(all)+(add)execute(x) \[script name\] -- thus changing the permission to allow everyone to execute a script. For a very helpful guide to chmod see [https://en.wikipedia.org/wiki/Chmod](https://en.wikipedia.org/wiki/Chmod)
