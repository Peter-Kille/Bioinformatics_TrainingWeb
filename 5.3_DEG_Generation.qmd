...
Processing data from counts to gene lists of significantly changed genes requires a number of complex steps - rather than code these individually we recommned using a 'wrapper' program that executes the steps within an automated workflow. NCBI Geo2R provides theis function for a selection of datasets archived in GEO [Geo2R Instrctions](https://peter-kille.github.io/Bioinformatics_TrainingWeb/5.2_GEO_Open_Data_Archive.html#c-geo2r-configured-unprocessed-raw-counts-and-group-metadata). For count data that we have genearted ourselves I am recommending using a wrapper called [SARTools](https://github.com/PF2-pasteur-fr/SARTools). This wrapper supports simple 2 state analsysis but generates all the plots for data exploration, normalisation, dispersion, MA/Volcano Plots and gene list. Also it generates a DESeq2 (or edgeR) object that can be used for more complex analysis.
...
mkdir 
cp -R ~/classdata/Session5/RNAseq-Analysis/Sartools-template-deseq2.r ~/mydata/
```
