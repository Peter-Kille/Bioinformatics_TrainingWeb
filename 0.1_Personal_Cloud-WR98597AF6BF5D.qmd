...
Cardiff University uses different VPNs for staff and students. You need to use the correct VPN or the connection will not work: the VPN software will install and you will think all is good but then it will not connect. To find the correct instructions, log in to the [intranet](intranet.cf.ac.uk) and search for VPN. Here are the current links which have installation instructions:
...
# RStudio (and jupiter notebook) Through Posit
...
![Posit Session](images/posit_session.png){width="\"60%"} Click on the + to create a new session.
...
![Rstudio Pro](images/Rstudio_pro.png){width="\"60%"} Leave teh default parameter as they are and start the session.
