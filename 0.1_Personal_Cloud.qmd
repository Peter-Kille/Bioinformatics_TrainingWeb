...
::: {.callout-note collapse="true"}
## HPC cluster access: Masters students, postgraduate researchers or staff
...
Select the session you wish to create - for this example we will create a RstudioPro Session. Leave the default parameters as they are and start the session.

![Rstudio Pro](images/Rstudio_pro.png){width="\"60%"} Tick the session you want to join and click on link to the "Rstudio Pro Session"
...
4.  Enter your password - this is you University single Sign on (SO) password

***You will NOT see any typing - this is a security feature, just continue typing***

![MobaXterm Password - this is your single sign on SO](images/MobaXterm_password.png){width="60%"}
...
# Linux Full Graphics Interface

We have the ability to interact with the HPC through a full OnDemand graphical interface - this is only recommended for graphical only programs and it uses significant amount of computation resources to power the interface leaving less resource to perform more interesting informatic tasks !!

To access this resource ensure you are logged into the vpn, open a browser of your choice and type in your server name

Example for people using sponsa would be:
...
![OnDemand Login Screen](images/OnDemandLogin.png){width="50%"}

Use you University username and password to log in to Open OnDemand .

The following screen should appear - if you see an error screen it probably means you are already login - in this case just paste `https://sponsa.bios.cf.ac.uk/pun/sys/dashboard` into browser window and the launch screen should appear.

![OnDemand Desktop Screen](images/OnDemandDesktop.png){width="50%"}

Click on Desktop and you will see the launch screen - note we have set the default to 8 hours - that means the desktop will close after that time - dont worry all data is still saved to server.

Now Click Launch

![OnDemand Launch Screen](images/OnDemandLaunch.png){width="50%"}

You will now see the Launch configuration screen - accept the defaults and click on Launch Desktop

![OnDemand Config Screen](images/OnDemandConfig.png){width="50%"}

You should not have a full graphical interface

![OnDemand Linux Graphical Interface](images/OnDemandLinux.png){width="50%"}

NB: Your ONDemand Interface by default maps your files to the gomphus hpc not sponsa. To navigate to files you are using in your personal cloud you will need to change directory to your mydata folder - here's the command you need.
