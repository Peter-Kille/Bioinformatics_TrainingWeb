...
You will need to run this every time that you log it. It will carve you out a little bit of space to interact freely with the machine whilst not interfering with others' work.
:::
...
MobaXterm is an excellent piece of software that supports SSH which enables you to create a SSH connection between your PC and the server. It has a built-in SFTP (Secure File Transfer Protocol) for file transfer, X-forwarding (graphical emulation) for using graphical applications remotely, and allows you to open multiple windows to your server. You can download the [MobaXTerm](https://mobaxterm.mobatek.net/download.html) from their website.

::: {.callout-note collapse="true"}
### Using MobaXterm on your personal PC

I suggest downloading and installing the 'Installer Addition' as it will fully install on your system - the portable version provides a simple executable (.exe) that can be placed on a data pen or used in machines where you do not have administrator privileges to install software.
:::
