...
To list what is in theses folders type `ls [foldername]` and you should display what is in each folder.
...
e.g. to list all fodlers in th working directory thats start with a capital S:
...
1.  List all the files in the folder classdata/Session1 that start with the letters sub

2.  List all the files in the folder classdata/Session1 that end with the letters fasta

3.  List all the files in your folder that start with sub, and end in fasta
...
## Changing your location and navigation

The command used to change directories (folder) is `cd`
...
To change to a directory one below you are in, just use the `cd` command followed by the subdirectory name i.e., if you have just logged in and want to move into the 'mydata' directory (inside your current directory), you could use:
...
```{bash}
cd /home/[your username]/classdata/
```

### Relative Navigation from home folder - the power of tilde (\~)

Rather than type the full path you can you a tilde `~` symbol to signify your home directory. You can use this as a jumping off point to navigate into your mydata folder
...
This should get you back to where you started..
...
cp ~/classdata/anyfile.txt ~/mydata/
```
...
```{bash}
cd ~/mydata
mv anyfile.txt Renamed_file.txt
```
...
2.  Copy the file CopyExercise.txt from `classdata` into `mydata`
