...
::: callout-important
## Exercise: Annotating with BLAST

Take one of your assembled genomes and identify orthologues using blast.

1.  Perform blastx analysis of your sequence against the swissprot protein database. Interrogate the output using 'less'.
...
You can now open the FASTA sequence you have used in the blast with Artemis and then overlay with the BLAST output file.

3.  Repeat the blastx analysis of your sequence only returning blast matches with an E value lower that 1E-10 and using all 4 threads.

4.  Repeat the blastx analysis of your sequence, only returning blast matches with an E value lower that 1E-10, using all 4 threads, limiting the outputs so that you keep to 100 and putting the outputs into the tabular format compatible with Artemis.
