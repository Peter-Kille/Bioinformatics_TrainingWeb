...
# Mitos2 - Only use for Mitochondrial annotation
...
```{bash}
    ~/classdata/REFS/script/selectSeqsAboveMinLength.pl
```
...
This code will extract your longest contig - if single contig no need to perform this step.,

**The risk associated with this option is that the gene you are intersted is not in the longest contig**
...
This code will remove the contig names (as long as they are named 2-9) for all accept the first contig and then remove any line breaks to ensure correct base nukbering for the annotation.
...
loop would be something like this (remember to edit loop for your files)
