...
#1. basename to remove folder path
#2. cut to select the first column after the name has been separated by "." (this removes .fastq.gz suffix).
#3. sed to replace read direction "_R1" with nothing. This leave just the sample name.

sample=$(basename ${file} | cut -f1 -d. | sed 's/_R1//')

printf "${sample}\t${sample}_R1.fastq.gz\t${sample}_R2.fastq.gz\n" >> samplelist.txt
