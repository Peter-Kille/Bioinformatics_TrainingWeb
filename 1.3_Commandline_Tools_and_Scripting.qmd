...
`zgrep` - search a conpressed file
...
`[your own script]` - any set of commands you want to run

`[varibles]` - any text or number you want to record and use
...
Bioinformatic is full of altruistic people who share their skills by share their scripts. Most scripts you find on the internet are written in \perl (\bioperl) or \python (\biophython) but many script use simple bash commands - this just lets you accurately repeat tasks. There are a few simple steps to making use of this fantastic resource.

## Varliables

A variable is like a box with a name written on the outside. You can put a value or text inside the box, and recall it later, or replace the contents with something else.

Here's a simple example. Note the bash syntax - the equals sign and " are important when defining the variable but when you recall it you need the `$` and `{}`
...
You can add fixed text to variables to create names of sample read files

```{bash}
name="SampleName"
echo ${name}_R1.fastq.gz
```

Or you can write to a file `>` or add to a file `>>`

```{bash}
name="SampleName"

#write to clean file
printf ${name}_R1.fastq.gz >> sample.txt

#or add to file
printf ${name}_R1.fastq.gz >> sample.txt

# Look what is in file
head sample.txt
```

You can also assign a variable to the output from code - the most easy example is to make a variable for the Present Working Directory (`pwd`) - note the smooth brackets around the code and the `$` infront of the smooth brackets.
...
## Script structure - what I think makes sense

There are many excellent computer science books about best practice about writing scripts - my advice is keep it simple.

Here's a few hints / recommendations:

- add lots of comments (starting with #) so you can remember what each line does later !! Be careful bash does not like these to be half way through commands

- I suggest a simple structure

  - Define variables
  - Load programs
  - Use loop (if appropriate)
  - Execute command
  - Unload programs or purge

- When writing complicated command break up the single line using line continuation symbol .\
  Add this at end of line and it tell bash the command to continue on the next line theerfore Treat the next line as part of the same command.

```{bash}

#this is not a real command just an example
some_command --input file1.txt --output results.txt --threads 8 --verbose

#indentations just make it look smart and things become easy to follow
some_command --input file1.txt \
             --output results.txt \
             --threads 8 \
             --verbose
```
...
## Loops loop to create a SampleList from rawdata folder {#sec-loops-loop-to-create-a-samplelist-from-rawdata-folder}
...
#set present working directory as the location of script....all folder will be related to this directory
workdir=$(pwd)
printf "SampleID\tforwardread\treverseread\n" > samplelist.txt 

#location of folder containing paired end sequence files - make sure your data is in a directory call rawdata in same location as your script.
rawdir=${workdir}/rawdata
# ls rawdata/*_R1.fastq.gz - this should list all files ending in _R1.fastq.gz

for file in ${rawdir}/*_R1.fastq.gz
do

#the file name are placed in variable $f - we can separate the name of the file away from the suffix (.fastq.gz) using this simple cut expression - the variable 'R1' now contains the file name with no suffix 
filename=$(basename ${file} | cut -f1 -d.)

#Sometimes we want the 'name' of the file without the direction suffix (_R1) - this expression creates a variable 'base' where the _R1 has been replace is nothing - ie removed 
sample=$(echo ${filename} | sed 's/_R1//')

printf "${sample}\t${sample}_R1.fastq\t${sample}_R1.fastq\n" >> samplelist.txt
...
## Program execution loop from samplelist {#sec-program-execution-loop-f-samplelist}

Print list of sample names to screen from samplelist file where the file has three tab deliminated columns encompassing: name, forwardread, and reverseread with first row having column headers.
...
#create variable for sample list saved in workdir
samplelist="samplelist.txt"

###  Load programs  ####
#remember to remove the # before use
#module load 

####  Configure you loop ####

#read in sample list
samplenames=$(cat ${samplelist} | cut -f1,1 | tail -n +2)

#loop to execute program on each sample
